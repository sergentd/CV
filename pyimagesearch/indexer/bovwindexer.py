...
class BOVWIndexer(BaseIndexer):
    def __init__(self, fvectorSize, dbPath, estNumImages=500, maxBufferSize=500,
        ...
        # open the HDF5 database for writing, init the dataset
        # within the group, the BOVW buffer list and the BOVW indexes
        # into the dataset
        ...
        # store the feature vector of the bag-of-visual-words, then
        # initialize the document frequency counts to be accumulated
        # and actual total number of images in the database
    ...
    def finish(self):
        # if the database have not been initialized, then the original
    ...
    def df(self, method=None):
        ...
        # otherwise, return the raw document frequency counts
        return self._df
