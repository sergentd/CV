...
def one_off_callback(trainIter, testIter, oneOff, ctx):
    def _callback(iterNum, sym, arg, aux):
        ...
        # compute the one-off metric for both the training and testing data
    ...
    # return the _callback method
    return _callback

    def _compute_one_off(model, dataIter, oneOff):
        # initialize the total number of samples along with
        # the number of correct maximum one off classifications
        total = 0
        correct = 0
        ...
        for (preds, _, batch) in model.iter_predict(dataIter):
            ...
            # loop over the predicted labels and ground-truth
            # labels in the batch
