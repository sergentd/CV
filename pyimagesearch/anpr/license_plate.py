...
LicensePlate = namedtuple("LicensePlateRegion",
    ["success", "plate", "thresh", "candidates"])

class LicensePlateDetector:
    def __init__(self, image, minPlateW=60, minPlateH=60, numChars=7,
        minCharW=40):
        # store the image to detect license plates in, the minimum width and
        # height, the number of characters to be detected in the license plate
        # and the minimum width of extracted chars
        self.image = image
    ...
    def detect(self):
        # detect the license plate regions
        lpRegions = self.detectPlates()

        # loop over the license plate regions
        for lpRegion in lpRegions:
            # detect the character candidates in the current lpregion
            lp = self.detectCharactedCandidates(lpRegion)
    ...
    def detectCharactedCandidates(self, region):
        ...
        # extract the Value component from the HSV color space and apply
        # adaptative thesholding to reveal the characters on the license plate
        ...
        # perform a connected component analysis and initialize the mask
        # to store the locations of the characters candidates
        ...
        for label in np.unique(labels):
            ...
            # components for the current label, then find contours in mask
            ...
            if len(cnts) > 0:
                ...
                # in the mask, then grab the bounding box for the contour
                c = max(cnts, key=cv2.contourArea)
                (boxX, boxY, boxW, boxH) = cv2.boundingRect(c)

                # compute the aspect ratio, solidity and height ratio for the
                # component
                aspectRatio = boxW / float(boxH)
                solidity = cv2.contourArea(c) / float(boxW * boxH)
                heightRatio = boxH / float(plate.shape[0])

                # determine if the aspect ratio, solidity and height of
                # contour pass the rules test
                ...
                if keepAspectRatio and keepSolidity and keepHeight:
                    # compute the convex hull of the contour and
                    # draw it on the character candidates mask
