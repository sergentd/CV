...
for i in np.arange(0, len(imagePaths), bs):
  ...
  # the list of actual images that will be passed through
  # the network for features extraction
  ...
  for (j, imagePath) in enumerate(batchPaths):
    ...
    # while ensuring the image is resized to 224 x 224
    image = load_image(imagePath, target_size=(224,224))
    image = img_to_array(image)

    # preprocess the image by expanding the dimensions
    # and by substracting the mean RGB pixel intensity from
    # ImageNet dataset
