...
# init a rectangular and a square structuring kernel
...
for imagePath in paths.list_images(args['images']):
  ...
  tresh = cv2.treshold(gradX, 0, 255, cv2.TRESH_BINARY | cv2.TRESH_OTSU)[1]
  
  # perform another closing operation, this time using the square kernel to close
  # gaps between lines of the MRZ, then perform a series of erosions to break apart connected components
  thresh = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, sqKernel)
  thresh = cv2.erode(thresh, None, iterations=4)
  
  # during thresholding, its possible that border pixels were included in the thresholding, so lets
  # set a 5% of the left and right border to zero
