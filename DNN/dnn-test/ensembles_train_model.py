...
# load the training and testing data, then scale it to
# the range [0, 1]
...
for i in np.arange(0, args["models"]):
  ...
  # train the network
  H = model.fit_generator(aug.flow(trainX, trainY, batch_size=64), validation_data=(testX, testY), epochs=40, steps_by_epoch=len(trainX) // 64, verbose=1)
  ...
  p = [args["output"], "report_model_{}.txt".format(i)]
