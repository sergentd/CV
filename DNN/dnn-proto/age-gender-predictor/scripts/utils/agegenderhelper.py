...
class AgeGenderHelper:
    def __init__(self, config):
        # store the configuration object and build the age bins
        # used for constructing class labels
        self.config = config
        self.ageBins = self.buildAgeBins()

    def buildAgeBins(self):
        # initialize the list of age bins based on the adience dataset
    ...
    def toGenderLabel(self, gender):
        # return 0 if gender is male, 1 if female
    ...
    def buildOneOffMappings(self, le):
        ...
        for (i, name) in enumerate(classes):
            ...
            # in the label encoder (unordered) list, then initialize
            # the index of the previous and next age groups adjacent
            ...
            # check to see if we need to compute the next adjacent age group
            if i < len(classes) - 1:
                next = np.where(le.classes_ == classes[i+1])[0][0]

            # construct a tuple that consists of the current age
            # bracket, the previous age bracket and the next age bracket
            oneOff[current] = (current, prev, next)

        # return the one-off mapping
        return oneOff
    ...
    def buildPathsAndLabels(self):
        ...
        for foldPath in foldPaths:
            # load the content of the folds files, skipping the header
            rows = open(foldPath).read().strip().split("\n")[1:]

            # loop over the rows
            for row in rows:
                # unpack the needed component of the row
                row = row.split("\t")
                (userID, imagePath, faceID, age, gender) = row[:5]

                # if the age of gender is invalid, ignore the sample
                ...
                # if the label is None, then ignore the sample
                if label is None:
                    continue
        ...
        # return a tuple of paths and labels
        return (paths, labels)
    ...
    def visualizeAge(agePreds, le):
        ...
        for (i, j) in enumerate(idxs):
            ...
            ageLabel = le.inverse_transform([j])[0]
