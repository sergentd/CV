...
# initialize the HOG descriptor with the list of data and labels
...
for (i, trnPath) in enumerate(trnPaths):
    ...
    # define the list of ROIs that will be described based on wether or not the
    # horizontal flip of the image should be used
    rois = (roi, cv2.flip(roi, 1)) if conf["use_flip"] else (roi,)
    
    # loop overthe ROIs
    for roi in rois:
        # extract the features from the ROI and update the list of features and labels
...
# grab the distraction image paths and reset the progressbar
pbar.finish()
...
print("[INFO] describing distraction ROI...")

# loop over the number of distraction images
