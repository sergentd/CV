...
class NeuralStyle:
  def __init__(self, settings):
    # store the settings dictionnary
    self.S = settings
    ...
    # load content image and style images, forcing
    ...
    # allocate memory for our output image, then combine
    # the content, style, and output into a single tensor so they
    # can be fed through the network
    ...
    # compare the feature reconstruction loss
    ...
    # weight each style layer (in proportion to the total
    ...
    for layer in self.S["style_layers"]:
      # grab the current style layer and use it to extract
      # the style features and output features from the
      # *style layer*
      styleOutput = layerMap[layer]
      styleFeatures = styleOutput[1, :,:,:]
      outputFeatures = styleOutput[2, :,:,:]
      
      # compare the style reconstruction loss
      T = self.styleReconLoss(styleFeatures, outputFeatures)
      styleLoss += (weight * T)
      
    # finish computing style loss, compute the total variational
    # loss and then compute the total loss that combines all three
    styleLoss *= self.S["style_weight"]
    tvLoss = self.S["tv_weight"] * self.tvLoss(self.output)
    totalLoss = contentLoss + styleLoss + tvLoss
    
    # compute the gradients out of the output image with
    # respect to loss
  ...
  def preprocess(self, p):
    # load the input image (while resizing it)
    # and preprocess it
  ...
  def deprocess(self, image):
    # reshape the image, then reverse the zero-centering
    # by adding back in the mean values across the ImageNet
    # training set
  ...
  def featureReconLoss(self, styleFeatures, outputFeatures):
    # the squared error between style and output features
  ...
  def styleReconLoss(self, styleFeatures, outputFeatures):
    ...
    # compute the scaling factore of the style loss, then finish
  ...
  def tvLoss(self, X):
    ...
    # the output page -- here we avoid border pixels to avoid artifacts
    (h, w) = self.dims
  ...
  def transfer(self, maxEvals=20):
    ...
    # placeholder array, slowly modified as we run L-BFGS
    # to apply style transfer
    ...
    for i in range(0, self.S["iterations"]):
      # run L-BFGS over the pixels in our generated image
      ...
      print("[INFO] end of iteration {}, loss: {}".format(i+1, loss))
