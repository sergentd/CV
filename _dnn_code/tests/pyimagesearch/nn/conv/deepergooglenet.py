...
class DeeperGoogLeNet:
  ...
  def conv_module(x, K, kX, kY, stride, chanDim,
	...
	# check to see if layer names were provided
	if name is not None:
	  convName = name + "_conv"
	  bnName   = name + "_bn"
	  act_name = name + "_act"
  ...
  def inception_module(x, num1x1, num3x3Reduce, num3x3, num5x5Reduce,
	...
	third = DeeperGoogLeNet.conv_module(third, num5x5, 5, 5, (1,1),
	  chanDim, reg=ref, name=stage + "_third2")
	  
	# define the fourth branch ot the inception module which is
	# the POOL projections
  ...
  def build(width, height, depth, classes, reg=0.0005):
	...
	# check to see if we are in "channels_first"
	if K.image_data_format() == "channels_first":
	  inpuShape = (depth, height, width)
	  chanDim = 1
	
	#define the model input, followed by a sequence of
	# CONV => POOL => (CONV *2) => POOL layers
	inputs = Input(shape=inpuShape)
